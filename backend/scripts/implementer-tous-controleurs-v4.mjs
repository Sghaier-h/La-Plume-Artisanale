/**
 * Script V4 - Implémentation automatique avec remplacement direct
 */
...
// Extraire le nom de fonction depuis une ligne
...
// Remplacer une fonction complète
function replaceFunc(content, funcName, newImpl) {
  // Pattern pour trouver la fonction complète avec son commentaire
  ...
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes(`export const ${funcName}`)) {
  ...
  // Vérifier que la fonction contient "Non implémenté"
  const funcContent = lines.slice(startIdx, endIdx + 1).join('\n');
  if (!funcContent.includes('Non implémenté') && !funcContent.includes('501')) {
    return { content, replaced: false };
  }
  
  // Remplacer
  const before = lines.slice(0, commentLine ? startIdx - 1 : startIdx).join('\n');
  const after = lines.slice(endIdx + 1).join('\n');
  const newContent = before + (before ? '\n' : '') + commentLine + newImpl + (after ? '\n' : '') + after;
...
console.log('🚀 Implémentation automatique de TOUS les contrôleurs génériques (V4)\n');
...
for (const { path: controllerPath, content, moduleName } of controllersToFix) {
  try {
    ...
    if (/create.*Non implémenté|create.*501/i.test(content)) {
      ...
      if (funcName) {
        const result = replaceFunc(newContent, funcName, createImpl(table, idField, funcName));
        if (result.replaced) {
    ...
    if (/update.*Non implémenté|update.*501/i.test(content)) {
      ...
      if (funcName) {
        const result = replaceFunc(newContent, funcName, updateImpl(table, idField, funcName));
        if (result.replaced) {
          newContent = result.content;
          modified = true;
    ...
    if (/delete.*Non implémenté|delete.*501/i.test(content)) {
      ...
      if (funcName) {
        const result = replaceFunc(newContent, funcName, deleteImpl(table, idField, funcName));
        if (result.replaced) {
          newContent = result.content;
          modified = true;
