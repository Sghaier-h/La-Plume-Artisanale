...
// integer pointing to modeles.id_modele) for all 1504 rows, but the modeles
...
//   - ON CONFLICT (id_modele) DO NOTHING keeps re-runs a no-op.
...
const main = async () => {
  ...
  for (const r of rows) {
    ...
    const ins = await pool.query(
      `INSERT INTO modeles
         (id_modele, name, description, code_modele, libelle, categorie,
          active, created_at, created_by)
       VALUES ($1, $2::varchar, $3::text, $4::varchar, $5::varchar, $6::varchar,
               true, NOW(), 1)
       ON CONFLICT (id_modele) DO NOTHING
       RETURNING id_modele`,
  ...
  // don't collide.
  const maxIdRow = await pool.query(`SELECT COALESCE(MAX(id_modele),0) AS m FROM modeles`);
  const maxId = Number(maxIdRow.rows[0].m) || 0;
  if (maxId > 0) {
    await pool.query(`SELECT setval('modeles_id_modele_seq', $1, true)`, [maxId]);
  }

  const total = await pool.query(`SELECT COUNT(*)::int AS n FROM modeles`);
  const linked = await pool.query(
    `SELECT COUNT(*)::int AS n FROM articles_catalogue a
       JOIN modeles m ON m.id_modele = a.id_modele`
  );
  ...
  console.log(`Sequence modeles_id_modele_seq set to: ${maxId}`);
