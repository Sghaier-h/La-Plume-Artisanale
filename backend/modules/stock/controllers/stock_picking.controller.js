/**
 * Stockpicking Controller - Version SQL directe
 */
...
const TABLE_NAME = 'livraisons';
const ID_FIELD = 'id_livraison';

// GET /api/stock/stock/picking - Liste tous les enregistrements
export const getStockPickings = async (req, res) => {
  try {
    ...
    if (search) {
      const searchConditions = ["numero_livraison"].map(field => 
    ...
    return handleError(res, error, 'getStockpicking');
  }
};

// GET /api/stock/stock/picking/:id - Récupère un enregistrement
export const getStockPicking = async (req, res) => {
  try {
    ...
    return handleError(res, error, 'getStockpickingById');
  }
};

// POST /api/stock/stock/picking - Crée un enregistrement
export const createStockPicking = async (req, res) => {
  try {
    ...
    return handleError(res, error, 'createStockpicking');
  }
};

// PUT /api/stock/stock/picking/:id - Met à jour un enregistrement
export const updateStockPicking = async (req, res) => {
  try {
    const { id } = req.params;
    const userId = getUserId(req) || 1;
    const data = req.body;
    ...
    return handleError(res, error, 'updateStockpicking');
  }
};

// DELETE /api/stock/stock/picking/:id - Supprime un enregistrement
export const deleteStockPicking = async (req, res) => {
  try {
    const { id } = req.params;
    const userId = getUserId(req) || 1;
    ...
    return handleError(res, error, 'deleteStockpicking');
  }
};
