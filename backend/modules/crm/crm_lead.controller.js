/**
 * Contrôleur CrmLeadController - Module crm
 */
...
// GET /api/crm/crm_lead - Liste tous les enregistrements
export const getCrmLead = async (req, res) => {
  try {
    const query = `SELECT * FROM crm_leads ORDER BY created_at DESC`;
    const result = await pool.query(query);
    return sendSuccess(res, result.rows, 'CrmLead récupérés avec succès');
  } catch (error) {
    return handleError(res, error, 'getCrmLead');
  }
};

// GET /api/crm/crm_lead/:id - Récupère un enregistrement
export const getCrmLeadById = async (req, res) => {
  try {
    const { id } = req.params;
    const query = `SELECT * FROM crm_leads WHERE id = $1`;
...
// POST /api/crm/crm_lead - Crée un enregistrement
export const createCrmLead = async (req, res) => {
  try {
    ...
    const query = `
      INSERT INTO crm_leads (${fields.join(', ')}, created_at, created_by)
...
// PUT /api/crm/crm_lead/:id - Met à jour un enregistrement
...
// DELETE /api/crm/crm_lead/:id - Supprime un enregistrement
export const deleteCrmLead = async (req, res) => {
  try {
    ...
    const checkActiveQuery = `
      ...
      WHERE table_name = 'crm_leads' AND column_name = 'active'
    `;
