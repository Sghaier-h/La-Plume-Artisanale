...
const ChefAtelierDashboard = () => {
  ...
  const getStatsOperation = (operationId) => {
    let total = 0;
    let enCours = 0;
    let termine = 0;
    let enAttente = 0;
          ...
          <div key={cmd.id} className="border rounded-lg p-6 bg-white shadow-sm">
            ...
            <div className="space-y-4">
              {cmd.articles.map((article, artIdx) => {
                const totalPremierChoix = article.suivis.reduce((sum, s) => sum + (s.qteLot - s.qteDeuxieme - s.qteRebut), 0);
                const totalDeuxieme = article.suivis.reduce((sum, s) => sum + s.qteDeuxieme, 0);
                const totalRebut = article.suivis.reduce((sum, s) => sum + s.qteRebut, 0);
                        ...
                        {article.suivis.map(suivi => (
                          <div key={suivi.numSuivi} className="border rounded-lg p-3 bg-gray-50">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
                              {Object.entries(suivi.operations).map(([opId, opData]) => {
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white rounded-lg p-4 shadow-md border-l-4 border-orange-500">
            ...
            <div className="text-2xl font-bold text-orange-600">
              {commandesEnCours.reduce((sum, cmd) => 
                sum + cmd.articles.reduce((s, art) => 
                  s + art.suivis.reduce((ss, suivi) => ss + suivi.qteDeuxieme, 0), 0), 0
              )}
            </div>
          </div>
