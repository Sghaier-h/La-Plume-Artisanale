...
const BonsRetourERP: React.FC = () => {
  ...
  const loadBons = async () => {
    ...
    try {
      ...
      const response = await api.get('/bons-retour', { params });
      const data = Array.isArray(response.data) ? response.data : response.data.data || [];
      setBons(data);
    } catch (error) {
      console.error('Erreur chargement bons de retour:', error);
      showError('Erreur', 'Impossible de charger les bons de retour');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEdit = (bon: BonRetour) => {
    setSelectedBon(bon);
    setShowForm(true);
    setViewType('form');
  };
  ...
  const handleSave = async (formData: any) => {
    ...
    try {
      if (selectedBon?.id) {
        await api.put(`/bons-retour/${selectedBon.id}`, formData);
        success('Bon de retour modifié', 'Les modifications ont été enregistrées');
      } else {
        await api.post('/bons-retour', formData);
        success('Bon de retour créé', 'Le bon de retour a été créé avec succès');
      }
      setShowForm(false);
      setSelectedBon(null);
      setViewType('list');
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Êtes-vous sûr de vouloir supprimer ce bon de retour ?')) {
      try {
        await api.delete(`/bons-retour/${id}`);
        success('Bon de retour supprimé', 'Le bon de retour a été supprimé avec succès');
        loadBons();
      } catch (error: any) {
      ...
      <div className="erp-content">
        <div style={{ display: 'flex', gap: '12px', marginBottom: '16px' }}>
          <div style={{ flex: 1, display: 'flex', gap: '8px' }}>
            ...
            <input
              type="text"
              placeholder="Rechercher un bon de retour..."
              value={search}
...
const BonRetourForm: React.FC<{
  ...
  return (
    <div className="erp-layout">
      <ERPHeader
        title={bon ? `Bon de Retour ${bon.name}` : 'Nouveau Bon de Retour'}
        breadcrumb={[
          { label: 'Ventes' },
          { label: 'Bons de Retour' },
        ...
        {bon && (
          <ERPStatusbar
            ...
            workflow={[
              ...
              { label: 'Retourné', value: 'done', color: 'done' }
            ]}
          />
        )}
            ...
            tabs={[
              {
                ...
                content: (
                  <div>
                    ...
                    <div className="erp-field">
                      <label className="erp-field-label">Motif du retour</label>
                      <textarea
