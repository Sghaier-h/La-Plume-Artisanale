...
SELECT 
    ...
    CASE 
        ...
        WHEN table_name IN ('of', 'postes_travail', 'machines', 'taches', 'ordres_fabrication', 'sous_of') THEN '🏭 Production'
        WHEN table_name IN ('stock', 'mouvements_stock', 'entrepots', 'stock_mp', 'stock_produits_finis', 'mouvements_mp') THEN '📦 Stock'
        WHEN table_name IN ('commandes', 'lignes_commande', 'articles_commande') THEN '🛒 Commandes'
        ...
        WHEN table_name IN ('controles_qualite', 'non_conformites', 'procedures_nc') THEN '✅ Qualité'
        WHEN table_name IN ('planning', 'planning_taches', 'planning_machines') THEN '📅 Planning'
        WHEN table_name IN ('alertes_actives', 'historique_alertes', 'types_alertes') THEN '🚨 Alertes'
        WHEN table_name IN ('logs_systeme', 'sync_queue') THEN '📝 Système'
        ELSE '📋 Autre'
    END as \"Catégorie\"
