#!/bin/bash
# Script de déploiement avec mot de passe intégré
# ⚠️ Pour développement uniquement - Changez le mot de passe en production !
...
if ! command -v psql &> /dev/null; then
    ...
    if psql -h $DB_HOST -p $DB_PORT -U $DB_USER -d $DB_NAME -f 01_base_et_securite.sql > /tmp/psql-01.log 2>&1; then
        ...
        echo "   ⚠️  Erreur sur 01_base_et_securite.sql (voir /tmp/psql-01.log)"
    fi
    ...
    if psql -h $DB_HOST -p $DB_PORT -U $DB_USER -d $DB_NAME -f 02_production_et_qualite.sql > /tmp/psql-02.log 2>&1; then
        ...
        echo "   ⚠️  Erreur sur 02_production_et_qualite.sql (voir /tmp/psql-02.log)"
    fi
    ...
    if psql -h $DB_HOST -p $DB_PORT -U $DB_USER -d $DB_NAME -f 03_flux_et_tracabilite.sql > /tmp/psql-03.log 2>&1; then
        ...
        echo "   ⚠️  Erreur sur 03_flux_et_tracabilite.sql (voir /tmp/psql-03.log)"
    fi
    ...
    if [ -f "04_mobile_devices.sql" ]; then
        ...
        if psql -h $DB_HOST -p $DB_PORT -U $DB_USER -d $DB_NAME -f 04_mobile_devices.sql > /tmp/psql-04.log 2>&1; then
            ...
            echo "   ⚠️  Erreur sur 04_mobile_devices.sql (voir /tmp/psql-04.log)"
        fi
    fi
...
echo ""
echo "⚠️  SÉCURITÉ : Changez le mot de passe en production !"
echo ""
