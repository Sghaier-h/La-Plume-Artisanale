/**
 * Script pour implémenter automatiquement les contrôleurs génériques
 * Version simplifiée avec remplacement direct
 */
...
function replaceFunc(content, funcName, newImpl) {
  ...
  return content.replace(pattern, newImpl);
}
...
for (const { path: controllerPath, content, moduleName } of controllersToFix) {
  try {
    ...
    if (/create.*Non implémenté|create.*501/i.test(content)) {
      const funcName = getFunctionName(content, 'create');
      const impl = createImpl(table, idField, funcName);
      const replaced = replaceFunc(newContent, funcName, impl);
      if (replaced && replaced !== newContent) {
        newContent = replaced;
        modified = true;
      }
    }
    ...
    if (/update.*Non implémenté|update.*501/i.test(content)) {
      const funcName = getFunctionName(content, 'update');
      const impl = updateImpl(table, idField, funcName);
      const replaced = replaceFunc(newContent, funcName, impl);
      if (replaced && replaced !== newContent) {
        newContent = replaced;
        modified = true;
    ...
    if (/delete.*Non implémenté|delete.*501/i.test(content)) {
      const funcName = getFunctionName(content, 'delete');
      const impl = deleteImpl(table, idField, funcName);
      const replaced = replaceFunc(newContent, funcName, impl);
      if (replaced && replaced !== newContent) {
        newContent = replaced;
        modified = true;
