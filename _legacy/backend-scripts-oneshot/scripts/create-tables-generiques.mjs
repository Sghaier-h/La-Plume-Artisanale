/**
 * Script pour créer automatiquement toutes les tables des contrôleurs génériques
 * Se connecte à la base de données et crée les tables manquantes
 */
...
// Configuration de la connexion (même config que db.js)
const poolConfig = {
...
// Mapping des contrôleurs vers leurs tables et ID fields
const tablesConfig = [
...
async function main() {
  ...
  for (const config of tablesConfig) {
    const { table, idField, controller } = config;
    ...
    try {
      ...
      if (exists) {
        ...
        alreadyExists++;
        results.push({ controller, table, status: 'exists', error: null });
      } else {
        // Créer la table
        ...
        created++;
        results.push({ controller, table, status: 'created', error: null });
      }
    } catch (error) {
      console.error(`❌ Erreur lors de la création de ${table}:`, error.message);
      errors++;
      results.push({ controller, table, status: 'error', error: error.message });
    }
  }
